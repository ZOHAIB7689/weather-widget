...
export default function WeatherWidget() {
  ...
  const handleSearch = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.weatherapi.com/v1/current.json?key=${process.env.NEXT_PUBLIC_WEATHER_API_KEY="9ea522b3d8814c76ad0150844240310"}&q=${trimmedLocation}`
      );
      if (!response.ok) {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-gradient-to-bl from-blue-200 to-orange-200">
      {/* Center the card within the screen */}
      <Card className="w-full max-w-md shadow-lg   duration-300 hover:scale-110 hover:shadow-zinc-900 mx-auto text-center">
